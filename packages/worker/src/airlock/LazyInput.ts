...
type Probe =
    ...
    | { kind: "container"; array: boolean }
    | { kind: "absent" };

interface Resolved { found: boolean; node: unknown }

function resolvePath(root: unknown, path: string[]): Resolved {
    let node = root;
    for (const key of path) {
        if (node === null || typeof node !== "object") return { found: false, node: undefined };
        if (FORBIDDEN.has(key)) return { found: false, node: undefined };
        if (!Object.prototype.hasOwnProperty.call(node, key)) return { found: false, node: undefined };
        node = (node as Record<string, unknown>)[key];
    }
    return { found: true, node };
}

function probe(r: Resolved): Probe {
    if (!r.found) return { kind: "absent" };
    const n = r.node;
    if (n !== null && typeof n === "object") return { kind: "container", array: Array.isArray(n) };
...
function dispatch(root: unknown, op: string, path: string[]): unknown {
...
export function installLazyBootstrap(context: ivm.Context): void {
...
export function bindLazyGlobal(context: ivm.Context, globalName: string, root: unknown): LazyBinding {
    const bridge = new ivm.Reference((op: string, path: string[]) => dispatch(root, op, path));
    ...
    return {
        release() {
            if (released) return;
            released = true;
            try { g.deleteSync(globalName); g.deleteSync(bridgeGlobal); } catch { /* isolate disposed */ }
            try { bridge.release(); } catch { /* already gone */ }
        },
    };
}
