...
const Tool: React.FC<Props> = ({ message }) => {
    ...
    const pill = (
        <div className={`
            flex flex-row gap-2 h-5 text-xs text-muted-foreground w-fit max-w-[90%]
            ${error ? "cursor-help" : ""}
        `}>
            ...
            <span className="font-mono h-auto my-auto text-[11px] font-bold truncate" title={message.data.tool_name}>
                {message.data.tool_name}
            </span>
            <div className="flex-1" />
            {!isSuccess && (
                <div className=' my-auto rounded-full h-5 w-5 flex'>
