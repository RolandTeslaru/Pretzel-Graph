...
const BlueprintItem = memo(({ blueprintId, ...props }: Props) => {
    ...
    <Tooltip.Root>
      <Tooltip.Trigger>
        <div
          ...
          {...props}
        >
          <Icon name={blueprint.data.ui.icon} className='w-[18px] h-[18px] my-auto ' />
      ...
      <Tooltip.Content side="left" className='max-w-[250px] gap-2' >
        <div className='flex flex-row justify-between'>
          <h4 className='font-semibold text-sm'>{blueprint.display_name}</h4>
          <Icon name={blueprint.data.ui.icon} className='w-[18px] h-[18px] text-muted-foreground' />
        </div>
...
BlueprintItem.displayName = 'BlueprintItem';
