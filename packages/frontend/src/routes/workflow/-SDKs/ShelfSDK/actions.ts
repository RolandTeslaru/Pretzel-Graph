    ...
    return {
        ...
        drawer: {
            open: (drawerId: Shelf.Drawer.Id) => {
                setState(s => { s.reducers.drawer.open(s, drawerId) });
            },
            close: (drawerId: Shelf.Drawer.Id) => {
                setState(s => { s.reducers.drawer.close(s, drawerId) });
            },
        ...
        setSection: (section: Shelf.Section) => setState(s => {
            s.reducers.setSection(s, section)
        }),

        searchFilter: {
            setQuery: (...props: Parameters<ShelfSDK.Reducers["searchFilter"]["setQuery"]> extends [any, ...infer Rest] ? Rest : never) =>
                setState(s => { s.reducers.searchFilter.setQuery(s, ...props) }),
        },
    } satisfies _ShelfActions
}

export type _ShelfActions = {
    loadSection: (section: Shelf.Section) => Promise<Record<Blueprint.Id, Blueprint>>;
