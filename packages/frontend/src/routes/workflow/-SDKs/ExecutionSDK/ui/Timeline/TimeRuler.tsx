...
const TimeRuler = ({ totalWidth, scale, totalDuration }: TimeRulerProps) => {
    const ticks = useMemo(() => {
        ...
        const interval = Execution.Recording.Timeline.tickIntervalMs(effectivePxPerMs)
    ...
    return (
        <div style={{ width: totalWidth, height: Execution.Recording.Timeline.RULER_H, position: "relative" }} className="select-none">
            {ticks.map(({ ms, x }) => (
                <div
                    key={ms}
                ...
                >
                    <span className="text-[10px] text-muted-foreground leading-none pt-1 px-0.5 whitespace-nowrap">
                        {Execution.Recording.Timeline.formatMs(ms)}
                    </span>
