...
export function buildTrackLayout(
    ...
    const inputPorts  = WorkbenchSDK.document.selectors.node.ports.getInputs(WorkbenchSDK.document, trackId).map(p => p.id) ?? []
    const outputPorts = WorkbenchSDK.document.selectors.node.ports.getOutputs(WorkbenchSDK.document, trackId).map(p => p.id) ?? []
