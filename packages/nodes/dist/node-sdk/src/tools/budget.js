...
(function (ToolBudget) {
    ...
    ToolBudget.DEFAULT_MAX_BYTES = 40_000;
    const encode = (value) => JSON.stringify(value) ?? "null";
    const bytes = (json) => Buffer.byteLength(json, "utf8");
    ...
    const reason = (kept, maxBytes) => kept === 0
     ...
     */
    function list(name, items, options = {}) {
        const maxBytes = options.maxBytes ?? ToolBudget.DEFAULT_MAX_BYTES;
        const build = (kept) => ({
            count: kept.length,
    ...
    function value(input, options = {}) {
        ...
        const longest = Math.max(...arrays.map(([, member]) => member.length));
        const build = (keep) => {
            const trimmed = { ...input };
            const dropped = {};
    ...
    ToolBudget.value = value;
    const isRecord = (input) => typeof input === "object" && input !== null;
    const withheld = (input, full, maxBytes, options) => {
        const fields = isRecord(input)
            ? Object.entries(input)
                .map(([key, member]) => [key, bytes(encode(member))])
...
})(ToolBudget || (exports.ToolBudget = ToolBudget = {}));
