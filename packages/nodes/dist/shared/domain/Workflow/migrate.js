...
exports.WORKFLOW_DATA_VERSION = void 0;
exports.migrateWorkflowDataToLatest = migrateWorkflowDataToLatest;
exports.WORKFLOW_DATA_VERSION = 2;
function migrateWorkflowDataToLatest(raw) {
    if (!raw || typeof raw !== "object")
        return raw;
    // Structural fat-node (v1) -> slim-node (v2). Skip if already slim.
    const data = raw.version === exports.WORKFLOW_DATA_VERSION
        ? { ...raw, nodes: { ...raw.nodes } }
        : migrateV1toV2(raw);
...
function liftAddedFieldExpressions(data) {
    for (const [nodeId, node] of Object.entries(data.nodes ?? {})) {
        if (!Array.isArray(node?.addedFields))
            continue;
        node.addedFields = node.addedFields.map((field) => {
            if (!field || !("isExpression" in field))
                return field;
...
function migrateV1toV2(raw) {
    ...
    return { ...raw, nodes, version: exports.WORKFLOW_DATA_VERSION };
}
function migrateNodeV1toV2(n) {
    const slim = { id: n.id, blueprintId: n.blueprintId };
    if (n.isDisabled)
        slim.isDisabled = true;
    if (n.dependency)
        slim.dependency = n.dependency;
    ...
    if (n.displayName !== undefined)
        ui.displayName = n.displayName;
    if (n.description)
        ui.description = n.description;
    if (n.isMinimized)
        ui.isMinimized = true;
    if (n.isFlipped)
        ui.isFlipped = true;
    ...
    const addedInputs = (n.inputs ?? []).filter((p) => p?.isAddedByUser);
    const addedOutputs = (n.outputs ?? []).filter((p) => p?.isAddedByUser);
    if (addedInputs.length)
        slim.addedInputs = addedInputs;
    if (addedOutputs.length)
        slim.addedOutputs = addedOutputs;
    return slim;
}
