...
- [ ] **decided — no delegate-aware scope guard.** Considered making `@Scoped`/`ScopedGuard` accept a `Principal.Delegate` so `WebhookTestController.register` (`/api/webhook/test/register`, `DelegateAuthGuard`) could drop its manual `assertDelegateWorkflow`. Rejected: the route stays **workflow-scoped with the manual assert**, `workflowId` moved from `@ZodBody` to a plain path param. The register's resource is keyed by `workflowId + path` on the webhook server (`Webhook.createId`), a per-workflow singleton executions contend for last-writer-wins — so the execution is incidental and there's no second consumer to justify the guard extension. The delegate merely proves the caller owns *an* execution of that workflow, which `assertDelegateWorkflow` already checks.
...
## Execution lifecycle gaps — spec not written

- [ ] **`ConsultationService.answer()` does not validate that the consultation is real.** `consultationId` is unverified, so a signal can be published for one that was never parked. Under the shared channel it lands and is ignored rather than vanishing into an unsubscribed channel — both silent. Asserting it against the execution's pending consultations belongs in `answer()`. Route scoping does not cover this: the guard authorizes `executionId` from the path, while `consultationId` is still an unchecked body field.
- [ ] **nothing emits `Execution.Event.Lifecycle.Suspended`.** `ExecutionService.suspend` awaits it, always times out, returns `success: false`, and never writes `status: 'suspended'` — though the abort does fire, so the execution really does stop.
