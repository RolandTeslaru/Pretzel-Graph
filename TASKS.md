...
- [ ] **backend:** `InternalAuthGuard` and `RuntimeNodeAuthGuard` → `Principal.Service` (a `service` name and nothing else). No separate `Principal.Internal` variant — it would carry no fields of its own.
...
- [ ] **backend (pre-tenancy blocker):** `RuntimeNodeAuthGuard` checks a single global static secret and produces **no identity** — it authenticates "a runtime", not *which* runtime. `internal-chat.controller.ts` then takes `executionId` from the request body and writes with `createServiceClient()`. `assertExecutionChat` correctly ties the chat to the execution's owner, so cross-user writes are blocked today. But under multi-tenancy every workspace's worker holds the same token, and workers are the one process running user-authored JavaScript. Scope the token per workspace (delivered alongside `WORKSPACE_ID`, same shape as the Redis ACL user) and have `assertExecutionChat` also require the execution to be in the caller's workspace.
- [ ] **backend:** `RuntimeNodeAuthGuard` compares the token with `!==` — swap for `crypto.timingSafeEqual`.
