...
- [x] **node-sdk:** added `UnstampedConsultationRequest<T>` beside `ConsultationAPI` — distributive `Omit<T, "id" | "startedAt">`, so a domain extending `Request` (HumanReview) keeps its own props through `consult`. Non-distributive `Omit` collapses a union to its common keys. Lives in node-sdk, not the domain: those two fields are only absent in transit to that one call, and nothing in `shared` consumes the type.
...
- [x] **shared:** grouped `Execution.Event`'s bare-literal members into sub-namespaces with prefixed literals, matching what `Node.*` (`node:started`) and `Recording.*` (`unit:started`) already did. `Started`…`Terminated` → `Event.Lifecycle.*` with `lifecycle:started` … `lifecycle:terminated`; `SessionPatch` → `Event.Session.Patch` with `session:patch`. One union and one `create` throughout — the namespaces organise, they do not split. `event.ts` imports `Session as SessionModule` to clear the name, the same dodge already used for `Recording as RecordingModule`. Call sites: 6 emits in `worker.ts`, 6 in `engine/*` + `turboGraph/apis.ts`, 5 `awaitEvent` type strings in `execution.service.ts`, and the frontend reducer's switch + `TERMINAL` set. `ExecutionSDK.AwaitedConfirmation` is a separate string union and is untouched, as is `Execution.Status`.
